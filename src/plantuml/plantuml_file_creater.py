...
def plantuml_diagram_creator_sub_domains(root_node, diagram_name, list_of_subdomains):
    ...
    while(que.isEmpty() != True):
        ...
        for child in curr_node.child_module:
            if child.name not in node_tracker:
                if check_if_module_should_be_in_filtered_graph(child.path, list_of_subdomains):
                    f = open(diagram_name_txt, "a")
                    f.write(diagram_type + child.name + "\n")
                    f.close()
    ...
    while(que.isEmpty() != True):
        ...
        for dependency in dependencies:
            x = 4
            if check_if_module_should_be_in_filtered_graph(dependency.path, list_of_subdomains) and check_if_module_should_be_in_filtered_graph(curr_node.path, list_of_subdomains):
                f = open(diagram_name_txt, "a")
                f.write(curr_node.name+"-->" + dependency.name + "\n")
                f.close()
